...
interface Kpi {
  ...
  delta: string
  spark: number[]
  /** Accent class for the dot + sparkline tint */
...
/** Conteos por día (últimos 14 días) usando `first_seen_at`, solo datos del API */
function countsPerRecentDay(
  events: ExposureEvent[],
  predicate: (e: ExposureEvent) => boolean,
  days = 14
): number[] {
  const now = Date.now()
...
function buildKpis(
  ...
  const severitySpark = countsPerRecentDay(
    events,
  ...
  return [
    {
      ...
      delta:
        critical + high === 0
          ? 'Sin critical/high en el dataset actual'
          : `${critical} críticos · ${high} altos`,
      spark: severitySpark,
    ...
    },
    {
      ...
      delta:
        online === false
          ? 'Sin datos: `NEXT_PUBLIC_API_URL` sin respuesta'
          : `${published} de ${total} eventos cargados tienen estado «published»`,
