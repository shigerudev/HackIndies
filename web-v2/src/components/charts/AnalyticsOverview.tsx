...
function SeverityTooltip({ active, payload }: { active?: boolean; payload?: TooltipPayload[] }) {
  if (!active || !payload?.length) return null
  const row = payload[0]?.payload && typeof payload[0].payload === 'object'
  ...
  const count = typeof row?.value === 'number' ? row.value : Number(payload[0]?.value ?? 0)
  return (
    <div className="glass glass-inner rounded-xl px-3 py-2 text-[12px]">
...
const VOLUME_VIEWS = [
  ...
  { id: 'bars_daily' as const, label: 'Barras · día a día', hint: 'Cada día es una columna' },
  { id: 'bars_weekly' as const, label: 'Barras · semana UTC', hint: 'Agrupa 7 días' },
]
...
function enrichDailyForStack(daily: { day: string; total: number; critical: number }[]) {
  return daily.map((d) => ({ ...d, otros: Math.max(0, d.total - d.critical) }))
}
...
function rollupWeeksFromDaily(daily: { day: string; total: number; critical: number }[]) {
  ...
  for (let i = 0; i < daily.length; i += 7) {
    ...
    rows.push({ label: `${chunk[0].day} → ${chunk[chunk.length - 1].day}`, total, critical, otros: Math.max(0, total - critical) })
  }
  return rows
}
...
function bucketByDay(events: ExposureEvent[]): { day: string; total: number; critical: number }[] {
  ...
  if (events.length > 0) {
    ...
    for (const e of events) oldestDay = Math.min(oldestDay, utcDayStart(new Date(e.first_seen_at).getTime()))
    const spanSinceOldestInclusive = Math.floor((utcDayStart(now) - oldestDay) / 86_400_000) + 1
    windowDays = Math.min(VOLUME_WINDOW_CAP_DAYS, Math.max(VOLUME_WINDOW_MIN_DAYS, spanSinceOldestInclusive))
  }
  ...
  for (let i = windowDays - 1; i >= 0; i--) {
    ...
    buckets.push({ day: `${dd}/${mm}`, total: 0, critical: 0 })
  }
  events.forEach((e) => {
...
function severityBreakdown(events: ExposureEvent[]) {
  ...
  const map: Record<ExposureEvent['severity'], keyof typeof counts> = { low: 'Baja', medium: 'Media', high: 'Alta', critical: 'Crítica' }
  events.forEach((e) => { const key = map[e.severity]; if (key) counts[key]++ })
  return Object.entries(counts).map(([name, value]) => ({ name, value }))
}

function institutionBreakdown(events: ExposureEvent[]) {
  const map = new Map<string, number>()
  events.forEach((e) => { const key = e.institution_name || e.institution_slug; map.set(key, (map.get(key) ?? 0) + 1) })
  return Array.from(map.entries()).map(([name, calls]) => ({ name, calls })).sort((a, b) => b.calls - a.calls).slice(0, 5)
}
...
export default function AnalyticsOverview() {
  ...
  const volumeYMax = useMemo(() => {
    let m = 1
    if (volumeView === 'bars_weekly') { for (const w of weeklyStacked) m = Math.max(m, w.total) }
    else { for (const b of dailyBuckets) m = Math.max(m, b.total, b.critical) }
    return m
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10">
        <div className="max-w-xl">
          ...
          <p className="mt-2 text-[12px] text-white/35">
            Volumen: ventana en días UTC (mín. {VOLUME_WINDOW_MIN_DAYS}, hasta {VOLUME_WINDOW_CAP_DAYS}).
          </p>
        ...
        <div className="flex gap-1 bg-white/[0.04] rounded-xl p-1 border border-white/[0.07] self-start">
          {TABS.map((tab) => (
            <button key={tab.id} onClick={() => setActive(tab.id)} className={`px-3.5 py-1.5 rounded-lg text-[12px] font-medium transition-all ${active === tab.id ? 'bg-white/10 text-white' : 'text-white/35 hover:text-white/55'}`}>
              {tab.label}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-3">
        {active === 'volume' && (
          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2">
            <span className="text-[10px] font-semibold tracking-[0.14em] uppercase text-white/30 shrink-0">Vista volumen</span>
            <div className="flex gap-1 flex-wrap">
              {VOLUME_VIEWS.map((v) => (
                <button key={v.id} type="button" title={v.hint} onClick={() => setVolumeView(v.id)} className={`px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all border border-transparent ${volumeView === v.id ? 'bg-white/10 text-white border-white/[0.12]' : 'text-white/40 hover:text-white/65 hover:bg-white/[0.04]'}`}>
                  {v.label}
                </button>
              ))}
            </div>
          ...
          {active === 'volume' && volumeView === 'area_daily' && (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dailyBuckets} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                <defs><linearGradient id="volGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="rgba(255,255,255,0.22)" /><stop offset="100%" stopColor="rgba(255,255,255,0)" /></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                <XAxis dataKey="day" tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10 }} tickLine={false} axisLine={false} interval={dailyBuckets.length > 40 ? Math.max(7, Math.floor(dailyBuckets.length / 14)) : 7} />
                <YAxis tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} domain={[0, volumeYMax]} />
          ...
          {active === 'volume' && volumeView === 'bars_daily' && (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailyStacked} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                <XAxis dataKey="day" tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 9 }} tickLine={false} axisLine={false} interval={dailyBuckets.length > 48 ? Math.max(6, Math.floor(dailyBuckets.length / 14)) : 4} />
                <YAxis tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} domain={[0, volumeYMax]} />
                <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(255,255,255,0.04)' }} />
                <Legend wrapperStyle={{ fontSize: 11 }} formatter={(value) => <span style={{ color: 'rgba(255,255,255,0.5)' }}>{value}</span>} />
                <Bar stackId="vol" dataKey="otros" name="No críticos" fill="rgba(255,255,255,0.32)" radius={[0, 0, 0, 0]} isAnimationActive />
          ...
          {active === 'volume' && volumeView === 'bars_weekly' && (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={weeklyStacked} margin={{ top: 4, right: 8, left: -20, bottom: 36 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                <XAxis dataKey="label" angle={weeklyStacked.length > 8 ? -28 : 0} textAnchor={weeklyStacked.length > 8 ? 'end' : 'middle'} height={weeklyStacked.length > 8 ? 54 : 32} tick={{ fill: 'rgba(255,255,255,0.35)', fontSize: 10 }} tickLine={false} axisLine={false} interval={0} />
                <YAxis tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} domain={[0, volumeYMax]} />
                <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(255,255,255,0.04)' }} />
                <Legend wrapperStyle={{ fontSize: 11 }} formatter={(value) => <span style={{ color: 'rgba(255,255,255,0.5)' }}>{value}</span>} />
          ...
          {active === 'severity' && (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={severities} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                <YAxis tick={{ fill: 'rgba(255,255,255,0.25)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} domain={[0, severityYMax]} />
                ...
                <Bar dataKey="value" name="Eventos" radius={[3, 3, 0, 0]} isAnimationActive>
                  {severities.map((_, i) => <Cell key={i} fill={PIE_WHITES[i % PIE_WHITES.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          )}
          ...
          {active === 'institutions' && (
            <div className="flex flex-col md:flex-row items-stretch gap-10 h-full">
              {institutions.length === 0 ? (
                <div className="flex items-center justify-center w-full text-[13px] text-white/35">Sin datos de instituciones todavía.</div>
              ) : (
                <>
                  <ResponsiveContainer width="40%" height="100%">
                    <PieChart>
                      <Pie data={institutions} cx="50%" cy="50%" innerRadius="52%" outerRadius="80%" dataKey="calls" isAnimationActive>
                        {institutions.map((_, i) => <Cell key={i} fill={PIE_WHITES[i % PIE_WHITES.length]} strokeWidth={0} />)}
                      </Pie>
                      <Tooltip content={({ active, payload }) => {
                        if (!active || !payload?.length) return null
                        const d = payload[0].payload as { name: string; calls: number }
                        return <div className="glass glass-inner rounded-xl px-3 py-2 text-[12px]"><p className="text-white/50 font-mono text-[11px]">{d.name}</p><p className="text-white/85 font-medium">{d.calls} eventos</p></div>
                      }} />
                    </PieChart>
                  </ResponsiveContainer>
